...
const previousYM = now.subtract(1, 'month').format('YYYY-MM');
...
const sources = [
  {
    ...
    listingUrls: [
      'https://www.whathifi.com/archive',
      'https://www.whathifi.com/news/archive',
      'https://www.whathifi.com/reviews/archive',
    ...
    sitemapUrls: [
      `https://www.whathifi.com/sitemap-${currentYM}.xml`,
      `https://www.whathifi.com/sitemap-${previousYM}.xml`,
      'https://www.whathifi.com/sitemap.xml'
    ],
    future: true
  },
  {
    ...
    sitemapUrls: [
      `https://www.techradar.com/sitemap-${currentYM}.xml`,
      `https://www.techradar.com/sitemap-${previousYM}.xml`,
  ...
  },
  {
    ...
    sitemapUrls: [
      `https://www.tomsguide.com/sitemap-${currentYM}.xml`,
      `https://www.tomsguide.com/sitemap-${previousYM}.xml`,
      'https://www.tomsguide.com/sitemap.xml'
    ],
    future: true
  }
];
...
const NON_ARTICLE_PATH_RE = /^\/$|\/author\/|\/category\/|\/tag\/|\/about\/?$|\/contact\/?$|\/archive\/?$|\/news\/archive\/?$|\/reviews\/archive\/?$|\/audio\/?$|\/hi-fi\/?$|\/headphones\/?$|\/speakers\/?$/i;
...
async function fetchText(url) {
  const res = await fetch(url, {
    headers: {
      'user-agent': 'Mozilla/5.0 (compatible; AudioContentSearcher/3.0; +https://github.com/teegar07/audio-content-searcher)',
...
function addCandidate(map, source, url, title='') {
  try {
    ...
    if (!prev || (!prev.title && title)) map.set(clean, { url: clean, title });
  } catch {}
}
...
function extractFeedCandidates(xml) {
...
function parseSitemap(xml) {
  ...
  return {
    urls: $('url').map((_, el) => ({
      ...
      lastmod: $(el).find('lastmod').first().text().trim()
    })).get(),
    sitemaps: $('sitemap').map((_, el) => ({
      loc: $(el).find('loc').first().text().trim(),
      lastmod: $(el).find('lastmod').first().text().trim()
    })).get()
  };
...
function sitemapPriority(loc) {
  ...
  if (s.includes(currentYM)) return 0;
  if (s.includes(previousYM)) return 1;
  if (/post|article|news|review|content|audio/.test(s)) return 2;
  return 3;
}

async function discoverFromSitemaps(source, candidateMap) {
  const queue = source.sitemapUrls.map(loc => ({ loc, priority: sitemapPriority(loc) }));
  const seen = new Set();
  let fetched = 0;
  ...
  while (queue.length && fetched < 60) {
    ...
    try {
      ...
      for (const row of parsed.urls) {
        const d = parseDate(row.lastmod);
        if (d && d.isBefore(cutoff.subtract(3, 'day'))) continue;
        addCandidate(candidateMap, source, row.loc);
      }

      for (const child of parsed.sitemaps) {
        if (!child.loc || seen.has(child.loc)) continue;
        const d = parseDate(child.lastmod);
        const priority = sitemapPriority(child.loc);
        if (priority > 2 && d && d.isBefore(cutoff.subtract(7, 'day'))) continue;
        queue.push({ loc: child.loc, priority });
      }
    } catch (e) {
...
async function discoverSource(source) {
  ...
  for (const feedUrl of source.feedUrls) {
    try {
      ...
      for (const c of extractFeedCandidates(xml)) addCandidate(candidateMap, source, c.url, c.title);
    } catch (e) {
  ...
  return [...candidateMap.values()].slice(0, 800);
}
